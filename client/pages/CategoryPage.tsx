...
export default function CategoryPage() {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (priceRange !== 'all') {
      filtered = filtered.filter(product => {
        const price = convertPrice(product.basePrice / 84.15, selectedCurrency.code)
        switch (priceRange) {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'trending': return (b.isTrending ? 1 : 0) - (a.isTrending ? 1 : 0) || b.reviews - a.reviews
        default: return 0
      }
    })
  ...
  const getCategoryTitle = (cat: string) => {
    switch (cat) {
      case 'football': return 'Football Collection'
      case 'anime': return 'Anime Universe'
  ...
  const getCategoryDescription = (cat: string) => {
    switch (cat) {
      case 'football': return 'Official jerseys, boots and gear from football legends worldwide'
      case 'anime': return 'Exclusive merchandise from your favorite anime series'
      case 'pop-culture': return 'Trending merchandise from music, movies, and pop culture'
      default: return 'Browse our complete collection'
    }
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-12">
          <div className="text-6xl mb-4">{categoryEmoji[category as keyof typeof categoryEmoji] || '🛍️'}</div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 p-4 bg-gray-50 rounded-lg">
          <div className="flex items-center space-x-4">
            ...
            >
              ...
              <option value="under-1000">Under {formatPrice(convertPrice(1000 / 84.15, selectedCurrency.code), selectedCurrency.code)}</option>
              <option value="1000-3000">{formatPrice(convertPrice(1000 / 84.15, selectedCurrency.code), selectedCurrency.code)} - {formatPrice(convertPrice(3000 / 84.15, selectedCurrency.code), selectedCurrency.code)}</option>
              <option value="over-3000">Over {formatPrice(convertPrice(3000 / 84.15, selectedCurrency.code), selectedCurrency.code)}</option>
            </select>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredAndSortedProducts.map((product) => {
            const convertedPrice = convertPrice(product.basePrice / 84.15, selectedCurrency.code)
