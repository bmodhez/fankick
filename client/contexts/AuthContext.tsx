...
interface AuthContextType {
  ...
  signup: (email: string, phone: string, name: string, password: string) => Promise<{ success: boolean; error?: string }>
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const signup = async (email: string, phone: string, name: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
  ...
  const verifyOTP = async (phone: string, otp: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (userToVerify) {
        ...
        setUsers(prev => prev.map(u => u.id === userToVerify.id ? verifiedUser : u))
