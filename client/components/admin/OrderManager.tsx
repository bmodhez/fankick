    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-primary">₹{(orderStats.revenue / 1000).toFixed(0)}K</p>
                ...
                {filteredOrders.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-gray-700">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          ...
                          {order.shipping.trackingNumber && (
                            <p className="text-xs text-gray-400">Track: {order.shipping.trackingNumber}</p>
                          )}
                        </div>
                      </td>
                        ...
                        <div className="space-y-1">
                          {order.items.map((item, index) => (
                            <div key={index} className="flex items-center space-x-2">
                              ...
                              <span className="text-xs text-gray-300">{item.quantity}x {item.name}</span>
                            </div>
                          ))}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          ...
                          <Badge className={`text-xs ${
                            ...
                            order.payment.status === 'pending' ? 'bg-yellow-600' :
                            order.payment.status === 'failed' ? 'bg-red-600' : 'bg-gray-600'
                          } text-white`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <p className="text-sm font-medium text-white">₹{order.payment.amount.toLocaleString()}</p>
                      </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-gray-800 border-gray-700">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-white">Order #{selectedOrder.orderNumber}</CardTitle>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <span className="text-lg">{selectedOrder.customer.flag}</span>
