...
function ProductCard({ product, onEdit, onDelete, onDuplicate }: ProductCardProps) {
      ...
      <CardContent className="p-4">
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-xs text-gray-400 capitalize">{product.category} • {product.subcategory}</p>
          </div>

          <div className="flex items-center justify-between">
            <div>
              <p className="text-lg font-bold text-primary">₹{product.basePrice.toLocaleString()}</p>
...
export function ProductManager() {
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'stock': return b.variants.reduce((sum, v) => sum + v.stock, 0) - a.variants.reduce((sum, v) => sum + v.stock, 0)
  ...
  const lowStockCount = products.filter(p => 
    p.variants.reduce((sum: number, v: any) => sum + v.stock, 0) < 10
  ).length

  const totalProducts = products.length
  const totalValue = products.reduce((sum, p) => sum + (p.basePrice * p.variants.reduce((vSum: number, v: any) => vSum + v.stock, 0)), 0)
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-white">{products.filter(p => p.isTrending).length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-gray-800 border-gray-700">
            ...
            <CardContent className="space-y-6">
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold text-gray-400 mb-2">Product Form</h3>
                <p className="text-gray-500">Detailed product creation form would be implemented here</p>
              </div>
            </CardContent>
          </Card>
        </div>
